<?php
	...
	function checkProjectPeriod(){
		global $mysql;
		$res = $mysql->DBGetAsMap("SELECT projectName,captainName,supervisorName,designerName,salesmanName,period from project where period is not null and isDeleted = 'false'");
		...
		foreach($res as $item){
			try{
				...
				echo "find project ".$item['projectName']."\n";
				...
				if($percentage == 30 || $percentage == 50 || $percentage == 80){
					...
					$menToSendMail = array();
					$subject = "【项目进度提示】".$item['projectName']." $percentage%";
					...
					if(count($menToSendMail) > 0){
						$names = array();
						foreach($menToSendMail as $key=>$val){
							array_push($names,"'".$val."'");
						}
						$names = join(",",$names);
						echo "mail to send:$names\n";
						$mails = $mysql->DBGetAsOneArray("SELECT mail from user where isDeleted = 'false' and mail is not null and name in ( $names )");
						if(count($mails) > 0){
							echo "send $subject to ".join(",",$mails)."\n";
							sendMail(join(",",$mails),$subject,$subject);
						}
					}
				}
				echo "\n\n";
